...
/*   Updated: 2018/12/12 01:20:15 by tcho             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int get_env_index(char **env, char *name)
{
	...
	while (env[i])
	{
		...
		if (ft_strcmp(name, split_result[0]) == 0)
			return (i);
		free_double(&split_result);
		i++;
	}
...
char *get_full_path(char **env, char *str, int index)
{
    ...
    if ((env_value = get_env_value(env, env_key)))
        return (ft_strnjoin(ft_strndup(str, start), env_value, ft_strrem(str, index)));
	return (NULL);
}
...
int expand(char **command, char **env)
{
	...
	if (command && *command)
	{
		while (command[i])
		{
			if ((ft_strcmp(command[i], "$PATH") == 0) && !g_display_path)
